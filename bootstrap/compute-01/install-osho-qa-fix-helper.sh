...
HELPER_DST="/usr/local/sbin/p2ops-osho-qa-fix"
SUDOERS_DST="/etc/sudoers.d/p2ops-osho-qa-fix"
RAW="https://raw.githubusercontent.com/prabhat324/p2-home-os/master/scripts/p2ops-osho-qa-fix"
APPLY_NOW="${OSHO_QA_FIX_APPLY_NOW:-0}"
...
p2ops ALL=(root) NOPASSWD: /usr/local/sbin/p2ops-osho-qa-fix status
p2ops ALL=(root) NOPASSWD: /usr/local/sbin/p2ops-osho-qa-fix apply
EOF
...
if [[ "$APPLY_NOW" == "1" ]]; then
    echo
    echo "=== APPLY SCOPED QA REPAIR ==="
    sudo -u p2ops sudo -n "$HELPER_DST" apply
fi

echo
echo "Scoped Osho QA helper installation complete."
