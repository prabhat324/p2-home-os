...
case "${operation}" in
  ...
  g50-02-provision)
    ...
    ;;
  dashboard-predeploy-backup-export)
    [[ "${target}" == "compute-02" ]] || { echo "dashboard-predeploy-backup-export is restricted to compute-02" >&2; exit 2; }
    playbook="playbooks/dashboard-predeploy-backup-export.yml"
    ;;
  ...
  media01-enroll)
    ...
    ;;
  media01-stage)
    [[ "${target}" == "media-01" ]] || { echo "media01-stage is restricted to media-01" >&2; exit 2; }
    playbook="playbooks/media01-stage.yml"
    ;;
  media01-acceptance)
    [[ "${target}" == "media-01" ]] || { echo "media01-acceptance is restricted to media-01" >&2; exit 2; }
    playbook="playbooks/media01-acceptance.yml"
    ;;
  media01-recover-worker)
  ...
  media01-status)
    ...
    ;;
  media01-production-focus)
    [[ "${target}" == "media-01" ]] || { echo "media01-production-focus is restricted to media-01" >&2; exit 2; }
    playbook="playbooks/media01-production-focus.yml"
    ;;
  ...
  media01-openmontage-install)
    ...
    ;;
  media01-openmontage-run)
    [[ "${target}" == "media-01" ]] || { echo "media01-openmontage-run is restricted to media-01" >&2; exit 2; }
    playbook="playbooks/media01-openmontage-run.yml"
    ;;
  *)
