#!/usr/bin/env python3
"""Autonomous review-only 4K worker. Never publishes or destructively edits source media."""
...
def recipe_key(source,manifest):
    st=source.stat();return hashlib.sha256(json.dumps({'source':str(source),'size':st.st_size,'mtime_ns':st.st_mtime_ns,'manifest':manifest,'pipeline':2},sort_keys=True).encode()).hexdigest()
...
def qa_with_repairs(r,source,output,review,work,duration,cfg):
    qa_report=review/'qa-report.json';result_path=work/'auto-repair-result.json'
    maximum=int(cfg.get('autonomy',{}).get('max_auto_repair_attempts',3));repairs=[]
    for index in range(maximum+1):
        try:
            r.run([sys.executable,APP/'qa_gate.py',output,'--source',source,'--profile',APP/'quality-profile.json','--report',qa_report],'QA_RUNNING',duration)
            return True,repairs
        except RuntimeError:
            qa=read(qa_report,{}) or {}
            if not qa:raise
            if index>=maximum:
                r.state('QA_REVIEW_REQUIRED',failures=qa.get('failures',[]),review=str(output),auto_repair_attempts=len(repairs),visual_baseline=qa.get('visual_baseline'));return False,repairs
            result_path.unlink(missing_ok=True)
            r.run([sys.executable,APP/'auto_repair.py',output,'--qa-report',qa_report,'--profile',APP/'quality-profile.json','--attempt',index+1,'--result',result_path],'AUTO_REPAIRING',duration)
            result=read(result_path,{}) or {};repairs.append(result)
            if not result.get('changed'):
                r.state('QA_REVIEW_REQUIRED',failures=qa.get('failures',[]),review=str(output),auto_repair_attempts=len(repairs),auto_repair=result,visual_baseline=qa.get('visual_baseline'));return False,repairs
    return False,repairs

def finish_review(r,source,output,review,work,duration,cfg,manifest):
    ...
    r.state('REVIEW_REQUIRED',qa='PASS',review=str(output),benchmark_only=manifest.get('purpose')=='benchmark',timings=str(work/'timings.json'),manual_review_required=True,auto_repair_attempts=len(repairs),auto_repairs=repairs,visual_baseline=qa.get('visual_baseline'))

def review_job(job):
    status=ROOT/'logs'/f'{job.name}.status.json';manifest=read(job/'project.json',{})
    ...
    with (work/'.lock').open('a+') as lock:
        ...
        try:
            ...
            timeline=job/'timeline.json';r.run([sys.executable,APP/'editorial.py','handoff',source,review,'--timeline',timeline],'BUILDING_HANDOFF',timeout=180)
            ...
            if (v['width'],v['height'])!=(3840,2160):cmd+=['-vf','scale_cuda=3840:2160:force_original_aspect_ratio=decrease:force_divisible_by=2,pad_cuda=3840:2160:(ow-iw)/2:(oh-ih)/2']
            ...
            partial.replace(output);finish_review(r,source,output,review,work,duration,cfg,manifest)
...
def main():
    ...
    while True:
        atomic(ROOT/'logs/worker-heartbeat.json',{'at':stamp(),'pid':os.getpid(),'pipeline':3})
