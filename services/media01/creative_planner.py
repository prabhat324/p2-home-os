...
def normalize_text(text):
    text=str(text or '')
    # Common ASR substitutions observed in Wasaga Beach productions. Graphics must use the
    # protected spelling even when transcript recognition drifts.
    text=re.sub(r'\b(?:Visaga|Vassaga|Vaseca|Vasaga|Wasega)\s+Beach\b','Wasaga Beach',text,flags=re.I)
    text=re.sub(r'\b(?:Visaga|Vassaga|Vaseca|Vasaga|Wasega)\b','Wasaga',text,flags=re.I)
    return text
...
def make_fact(flag,source):
    text=' '.join(words(normalize_text(flag.get('text','')))[:22])
...
def make_asset(asset):
    return {'start':float(asset['start']),'end':float(asset['end']),'kind':asset.get('kind','image'),'approved':True,'asset':asset['path'],'source':asset.get('source','Project-provided asset'),'caption':normalize_text(asset.get('caption','')),'generated_by':'creative_planner'}

def make_graphic(item):
    kind=item.get('kind')
    if kind not in {'fact_card','graph'}:raise ValueError('graphic_events supports fact_card or graph')
    ...
    if kind=='fact_card':
        ...
        e['title']=normalize_text(item.get('title',''))
        e['unit']=normalize_text(item.get('unit',''))
        e['data']=[{'label':normalize_text(x.get('label','')),'value':float(x['value'])} for x in item.get('data',[])]
...
def segment_near(transcript,second):
    ...
    return normalize_text(str(best.get('text','')).strip())

def build(manifest,report,transcript,duration):
    mode=manifest.get('mode','explainer')
    out={'version':4,'mode':mode,'events':[],'chapters':manifest.get('chapters',[]),'creative_policy':{},'asset_requests':[]}
    if mode=='podcast':
    ...
    # Do not duplicate a curated graphic with an auto fact card at the same moment.
    curated_ranges=[(e['start']-2,e['end']+2) for e in events if e.get('kind') in {'fact_card','graph'}]
    for flag in report.get('fact_check_flags',[]):
        ...
        elif not source and not any(a<=sec<=b for a,b in curated_ranges):
            out['asset_requests'].append({'second':round(sec,2),'kind':'verified_fact_source','context':normalize_text(str(flag.get('text','')).strip()),'reason':'numeric/date/currency claim needs a primary or authoritative source before graphic treatment'})
    ...
    while t<duration-8:
        ...
        t+=25.0
    priority={'image':5,'document':5,'newspaper':5,'broll':5,'fact_card':4,'graph':4,'zoom':1}
    clean=[]
