...
def validate(t,duration):
    last=0
    supported={'broll','graph','zoom','fact_card','image','document','newspaper'}
    for e in sorted(t.get('events',[]),key=lambda x:x['start']):
        ...
        if e['kind'] in {'broll','image','document','newspaper'} and (not e.get('asset') or not e.get('source')):raise ValueError('Visual asset needs asset path and source/license reference')
...
def handoff(source,out,t):
    ...
    (out/'HANDOFF.txt').write_text('Original source preserved. Relink source media on the Mac. OTIO source clip and chapter markers verified by library round trip; DaVinci import must be checked on the Mac. Custom events are rendered by media01 and are not native Resolve effects. Captions may be burned for podcast mode and remain sidecar analysis artifacts too. Full timeline, political neutrality, spelling and source review are required.\n')
...
def graph(e,path):
    ...
    im=Image.new('RGB',(3840,2160),'#102132');d=ImageDraw.Draw(im);title=font(size=88);body=font(size=48)
    rows=e['data']
...
def fact_card(e,path):
    from PIL import Image,ImageDraw
    im=Image.new('RGB',(3840,2160),'#0E1C2A');d=ImageDraw.Draw(im);title=font(size=92);body=font(size=44)
    d.rounded_rectangle((260,360,3580,1720),radius=60,fill='#152B40',outline='#4A718F',width=4)
    words=str(e['title']).split();lines=[];line=[]
    for w in words:
        if len(' '.join(line+[w]))>42 and line:lines.append(' '.join(line));line=[w]
        else:line.append(w)
    if line:lines.append(' '.join(line))
    y=620
    for ln in lines[:4]:d.text((430,y),ln,font=title,fill='white');y+=135
    d.text((430,1530),'Source: '+str(e['source'])[:105],font=body,fill='#C8D7E4');im.save(path)

def still_card(e,path):
    from PIL import Image,ImageDraw
    src=Path(e['asset'])
    im=Image.open(src).convert('RGB');im.thumbnail((3280,1640))
    canvas=Image.new('RGB',(3840,2160),'#101820');x=(3840-im.width)//2;y=(1840-im.height)//2;canvas.paste(im,(x,y));d=ImageDraw.Draw(canvas);body=font(size=42)
    if e.get('caption'):d.text((260,1840),str(e['caption'])[:130],font=body,fill='white')
    label={'newspaper':'Source / article','document':'Source / document','image':'Source / image'}.get(e['kind'],'Source')
    d.text((260,1940),f"{label}: {str(e['source'])[:110]}",font=body,fill='#C7D5DF');canvas.save(path)
...
def render(source,out,t):
    ...
    for i,e in enumerate(parts):
        ...
        elif kind in {'image','document','newspaper'}:
            asset=out/f'still-{i}.png';still_card(e,asset);args+=['-loop','1','-i',asset]
...
def proxy(source,out):
    run(['ffmpeg','-nostdin','-y','-v','error','-hwaccel','cuda','-hwaccel_output_format','cuda','-i',source,'-vf','scale_cuda=1280:720','-c:v','h264_nvenc','-preset','p4','-cq','25','-c:a','aac','-b:a','128k',out/'proxy.partial.mp4']);(out/'proxy.partial.mp4').replace(out/'proxy-720p.mp4')
    duration,_=metadata(source);run(['ffmpeg','-nostdin','-y','-v','error','-i',source,'-vf',f'fps=1/{max(1,duration/24)},scale=480:-2,tile=4x6','-frames:v','1',out/'contact-sheet.jpg'])
